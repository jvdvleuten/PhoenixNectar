...
public actor Socket {
  ...
  private func scheduleReconnectIfNeeded(trigger: ReconnectTrigger) async {
    ...
    scheduler.cancel(reconnectToken)
    reconnectToken = scheduler.scheduleOnce(after: delay, tolerance: nil) { [weak self] in
      guard let self else { return }
      do {
  ...
  private func startHeartbeatLoop() {
    ...
    heartbeatToken = scheduler.scheduleRepeating(
      every: configuration.connectionPolicy.heartbeatInterval,
      tolerance: nil
    ) { [weak self] in
      guard let self else { return }
  ...
  private func scheduleRejoinRetry() {
    ...
    scheduler.cancel(rejoinRetryToken)
    rejoinRetryToken = scheduler.scheduleOnce(after: delay, tolerance: nil) { [weak self] in
